// ============================================================
// Nộp ảnh & thông tin nhân viên — logic chính (bản demo)
// ============================================================
...
const els = {
  ...
  expiredDot: document.getElementById("expired-dot"),
  expiredHan: document.getElementById("expired-han"),
...
function init() {
  ...
  if (today > deadline) {
    els.expiredDot.textContent = tokenInfo.dotThu;
    els.expiredHan.textContent = formatDateVN(tokenInfo.hanNop);
  ...
  els.infoHan.textContent = formatDateVN(tokenInfo.hanNop);
...
function addFamilyRow() {
  ...
  // đổ danh sách "mối quan hệ" và "quốc tịch" từ config.js
  const relationSelect = row.querySelector(".f-relation");
  relationSelect.appendChild(new Option("-- Chọn --", ""));
  RELATION_OPTIONS.forEach((opt) => relationSelect.appendChild(new Option(opt.label, opt.value)));

  const nationalitySelect = row.querySelector(".f-nationality");
  nationalitySelect.appendChild(new Option("-- Chọn --", ""));
  NATIONALITY_OPTIONS.forEach((n) => nationalitySelect.appendChild(new Option(n, n)));
...
// Toggle 2: "Có người thân đang sống tại Nhật không?"
...
function openCropModal(dataUrl) {
  ...
  cropper = new Cropper(els.cropImage, {
    aspectRatio: 3 / 4, // rộng:cao = 3:4 theo quy cách ISA (4cm cao x 3cm rộng)
    viewMode: 1,
    dragMode: "move",
    autoCropArea: 0.9,
    background: false,
...
els.btnCropConfirm.addEventListener("click", async () => {
  ...
  els.cropFaceStatus.textContent = "Đang kiểm tra ảnh…";
  ...
  if (faceResult.ok) {
    canvas.toBlob(
      (blob) => {
        ...
        els.photoStatus.textContent = "✓ Ảnh hợp lệ, đã sẵn sàng để nộp";
    ...
    els.cropFaceStatus.textContent = faceResult.message;
...
async function checkFaceCount(canvas) {
  if (!faceModelsReady) {
    // Không chặn nộp ảnh nếu model lỗi tải (demo/offline) — chỉ cảnh báo.
  ...
  try {
    ...
    if (detections.length === 0) {
      return { ok: false, message: "✗ Không nhận diện được khuôn mặt nào. Vui lòng chụp lại, để mặt rõ trong khung." };
    }
    if (detections.length > 1) {
      return { ok: false, message: "✗ Phát hiện nhiều hơn 1 khuôn mặt trong ảnh. Ảnh chỉ được có 1 người." };
    }
    return { ok: true, message: "" };
  } catch (err) {
    console.error(err);
    return { ok: true, message: "" }; // lỗi kỹ thuật -> không chặn, HR duyệt tay
  }
}
...
els.mainForm.addEventListener("submit", async (e) => {
  ...
  els.btnSubmit.textContent = "Đang nộp…";
  ...
  try {
    ...
    els.submitError.textContent =
      "Có lỗi khi nộp (kiểm tra lại kết nối mạng, hoặc cấu hình Supabase trong js/config.js). Chi tiết: " +
      (err.message || err);
    els.submitError.classList.remove("hidden");
    els.btnSubmit.disabled = false;
    els.btnSubmit.textContent = "Nộp ảnh & thông tin";
  }
});
