...
function openCropModal(dataUrl) {
  ...
  cropper = new Cropper(els.cropImage, {
    ...
    // Khung không đổi vị trí/kích thước sau khi tạo xong (2 dòng ở trên), nên chỉ
    // cần vẽ viền canh đầu-vai đúng 1 lần lúc khung đã sẵn sàng.
    ready: renderCropGuide,
  });
}
...
// ---------------------------------------------------------------
// 3b. Viền gợi ý canh đầu — vai bên trong khung crop (giống khung ảnh thi JLPT)
// Chỉ để NGƯỜI DÙNG tự canh mắt thường — không đo đạc, không chặn nộp ảnh.
// Khung crop cố định (không di chuyển/đổi cỡ) nên chỉ cần định vị viền 1 lần
// ngay khi Cropper báo "ready", không cần cập nhật lại sau đó.
// ---------------------------------------------------------------
function renderCropGuide() {
  removeCropGuide();
  if (!cropper || !els.cropArea) return;
  const box = cropper.getCropBoxData();

  const guide = document.createElement("div");
  guide.id = "crop-guide";
  guide.className = "crop-guide";
  guide.style.left = box.left + "px";
  guide.style.top = box.top + "px";
  guide.style.width = box.width + "px";
  guide.style.height = box.height + "px";
  // viewBox 300x400 = đúng tỉ lệ 3:4 của khung, nên vẽ toạ độ cố định là khớp mọi
  // kích thước thật. Hình bầu dục = đầu, 2 nét cong 2 bên = vai.
  guide.innerHTML =
    '<svg viewBox="0 0 300 400" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">' +
    '<ellipse cx="150" cy="150" rx="80" ry="97" fill="none" stroke="rgba(255,255,255,0.9)" stroke-width="3" stroke-dasharray="7 6" stroke-linecap="round" />' +
    '<path d="M 44 238 C 18 262, 4 322, 0 400" fill="none" stroke="rgba(255,255,255,0.9)" stroke-width="3" stroke-dasharray="7 6" stroke-linecap="round" />' +
    '<path d="M 256 238 C 282 262, 296 322, 300 400" fill="none" stroke="rgba(255,255,255,0.9)" stroke-width="3" stroke-dasharray="7 6" stroke-linecap="round" />' +
    "</svg>";
  els.cropArea.appendChild(guide);
}

function removeCropGuide() {
  const old = document.getElementById("crop-guide");
  if (old) old.remove();
}
